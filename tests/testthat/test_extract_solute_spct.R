...
test_that("solute_spct", {
  ...
  expect_equal(class(my.spct)[1], "solute_spct")
  expect_equal(class(my.spct[ 1, ])[1], "solute_spct")
  expect_equal(class(my.spct[ FALSE, ])[1], "solute_spct")
  expect_equal(class(my.spct[ , -1])[1], "numeric")
  expect_equal(class(my.spct[ , 1])[1], "integer")
  expect_equal(class(my.spct[ FALSE, 1])[1], "integer")

  expect_equal(class(my_z.spct)[1], "solute_spct")
  expect_equal(class(my_z.spct[ 1, ])[1], "solute_spct")
  expect_equal(class(my_z.spct[ FALSE, ])[1], "solute_spct")
  expect_equal(class(my_z.spct[ , 1])[1], "integer")
  expect_equal(class(my_z.spct[ , -1])[1], "tbl_df")
  expect_warning(my_z.spct[ , -2])
  expect_equal(suppressWarnings(class(my_z.spct[ , -2])[1]), "solute_spct")
#  expect_equal(class(my_z.spct[ FALSE, -2])[1], "filter_spct")
#  expect_warning(my_z.spct[ FALSE , -2])

  expect_equal(getKType(my_z.spct), getKType(my.spct))
  expect_equal(getKType(my.spct[1:10, ]), getKType(my.spct))
  expect_equal(getKType(my.spct[FALSE, ]), getKType(my.spct))
  expect_equal(getKType(my_z.spct[ , -3]), getKType(my.spct))
  expect_equal(getKType(my_z.spct[-(1:5) , -3]), getKType(my.spct))
  expect_equal(getKType(my_z.spct[ , -3]), getKType(my.spct))

...
#'
#' @usage irradiance(w.length, s.irrad, w.band=NULL, unit.out=NULL, unit.in="energy", check.spectrum=TRUE)
#' 
...
irradiance <- 
  function(w.length, s.irrad, w.band=NULL, unit.out=NULL, unit.in="energy", check.spectrum=TRUE){
    ...
    if (check.spectrum && !check_spectrum(w.length, s.irrad)) return(NA)

#' Split a spectrum into contiguous bands and calculate energy irradiance 
#' from spectral (energy) or photon irradiance.
#'
#' This function returns the energy irradiance for a series of contiguous wavebands
#' from a radiation spectrum. The returned values can be either absolute or relative to their
#' sum.
#'
#' @usage split_energy_irradiance(w.length, s.irrad, cut.w.length=range(w.length), unit.in="energy", 
...
split_energy_irradiance <- function(w.length, s.irrad, cut.w.length=range(w.length), unit.in="energy", scale="absolute", 
                 check.spectrum=TRUE, use.cached.mult=FALSE, use.hinges=NULL)
{
    return(split_irradiance(w.length, s.irrad, cut.w.length=cut.w.length, unit.out="energy", unit.in=unit.in, scale=scale, 

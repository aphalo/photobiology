...
%\VignetteDepends{knitr, photobiology, lubridate}
...
\newcommand{\GGSP}{\textsf{'ggspectra'}\xspace}
...
facilitate the calculation of many different quantities that can be derived from spectral irradiance data. The core package in this suite is called \PB, and is the package described here. Other specialized packages for quantification of ultraviolet radiation and visible radiation (\PBWB), plant photoreceptors and other plant photobiology related calculations (\PBPla), example spectral data for filters (\PBFLT), lamps (\PBLA), LEDs (\PBLD), sunlight (\PBSU), broadband sensors (\PBSN) and for exchange of data in `foreign' formats (\PBIO) are part of the suite. One additional package, (\GGSP), implements facilities for plotting spectral data built on top of package `ggplot2', providing both ggplot statistics in addition to specializations of \texttt{plot()}. All packages that can comply with the repository rules have been submitted or will in coming months be submitted to CRAN (Comprehensive R archive network). The packages not yet in CRAN, and development versions of those already in CRAN, are available through a CRAN-like repository at \url{https://www.r4photobiology.info/R/} while the Web site at \url{http://www.r4photobiology.info} provides news about the development of the suite and some additional information. Each package has its own public Git repository at my Bitbucket account (\url{https://bitbucket.org/aphalo/}) from where the source code of the current and previous versions can be cloned.

Package \PB provides two sets of functions for many operations: low-level functions programmed following a functional paradigm, and higher-level functions using an object-oriented paradigm. The former functions take as arguments numeric vectors and are sometimes faster. The later ones take `spectra' objects as arguments, are easier to use, and at least at the moment, to some extent slower. For everyday use `spectra' objects are recommended, but when maximum performance or flexibility in scripts is desired, the use of the functions taking numeric vectors as arguments may allow optimizations that are not possible with the object-oriented functions. The differences in performance becomes relevant only in extreme cases such as processing in a single script tenths of thousands of spectra. In this vignette we emphasize the use of the object-oriented classes and methods.

\section{Installation and use}

The functions in package \PB are
made available by installing the package \verb!photobiology! (once) in a computer system and
...
In addition to the classes for storing individual spectra, classes for storing collections of spectra are defined. These classes are derived from class \texttt{list} and can contain member spectra of different lengths and measured at different wavelength values.
...
\begin{table}
\caption[Classes for spectral data]{Classes for spectral data. In addition to the required variables listed in the table, additional arbitrary variables are partly supported---some operations will not include them in returned values to avoid ambiguity and other possible conflicts. In addition to the attributes listed, all spectral objects have attributes \texttt{multiple.wl}, \texttt{normalized}, \texttt{scaled}, \texttt{when.measured}, \texttt{where.measured}, \texttt{what.measured} plus the normal attributes of \texttt{data\_frame} objects including \texttt{comment}.}\label{tab:spct:classes}
\centering
\begin{footnotesize}
...
\addlinespace
raw\_spct & w.length, counts & instr.desc, instr.settings, linearized \\
\addlinespace
cps\_spct & w.length, cps & instr.desc, instr.settings, linearized \\
\addlinespace
source\_spct & w.length, s.e.irrad & time.unit, bswf \\
             & w.length,  s.q.irrad & time.unit, bswf \\
\addlinespace
filter\_spct & w.length, Tfr & Tfr.type \\
             & w.length, A & Tfr.type \\
\addlinespace
...
\addlinespace
response\_spct & w.length, s.e.response & time.unit \\
...
the same observation event. In other words, the storage format is `tidy' as defined by Hadley Wickham.
...
An assumption of the package is that wavelengths are always expressed in nanometres ($1~\Unit{nm} = 1 \cdot 10^{-9}\,\Unit{m}$). If the data to be analysed uses different units for wavelengths, e.g.\ Ångstrom ($1~\textrm{Å} = 1 \cdot 10^{-10}\,\Unit{m}$), the values need to be re-scaled before creating objects of the spectral classes. The same applies to all spectral quantities, as there is an expectation in every case, of using base SI units for expression. Table \ref{tab:spct:units} lists the units of expression for the different variables listed in  Table  \ref{tab:spct:classes} and the metadata attributes that may determine variations in the expression of the quantities.
...
Energy irradiance is assumed to be expressed in \watt and photon irradiances in \mol, that is to say using second as unit for time. This is the default, but it is possible to set the unit for time to day in the case of \texttt{source\_spct} objects.

The default time unit used is \emph{second}, but \emph{day} and \emph{exposure} can be used by supplying the arguments \texttt{"day"} or \texttt{"exposure"}\footnote{The meaning of \texttt{"exposure"} is the total exposure  time, in other words, fluence instead of irradiance.} to a parameter of the constructor of \texttt{source\_spct} objects. In addition to these character constants objects of class \texttt{lubridate:duration} are also accepted.
...
If, for example, the spectral irradiance data output by a spectroradiometer is expressed in \mwattnm, and the wavelengths are in Ångstrom then to obtain correct results when using any of the packages in the suite, we need to rescale the data when creating a new object.
...
Before giving examples of how to construct objects to store spectral data we show how to query the class of an object, and how to query the class of a spectrum. Consistently with R, the package provides `is' functions for querying the type of spectra objects. The only `unusual' function provided as another name for \texttt{is.generic\_spct} is \texttt{is.any\_spct()}.
...
In addition function \texttt{class\_spc()} returns directly the spectrum-related class attributes. It filters out from the output of \texttt{class()} the underlying classes inherited.
...
There are basically two different approaches to the creation of spectra by users, a constructor similar to \texttt{data.frame} constructor that takes vectors as arguments, and a constructor that converts \texttt{list}
...
Here we briefly describe the `as' constructor functions for spectra. In the first example we create an object to store spectral irradiance data for `light source', by first creating a data frame, and creating the spectral object as a copy of it. In the example below we supply a single value, 1, for the spectral irradiance. This value gets recycled as is normal in R, but of course in real use it is more usual to supply a vector of the same length as the \texttt{w.length} vector.
...
Spectral objects have several attributes used to store metadata, such as the time unit used or type of spectral quantity. Some attributes are meaningful for all the classes of spectra defined in the package. These are \emph{time of measurement} using attribute \texttt{"when.measured"}, \emph{place of measurement} using attribute \texttt{"where.measured"} and free-text \emph{comments}. One can set and get comments stored in spectra by means of base R's \texttt{comment()} and \verb|comment() <-| functions. Some of the functions in this package append additional information to comments or merge comments.
...
Functions \texttt{setWhereMeasured()} and \texttt{getWhereMeasured()} are used for setting or getting a geocode as a \texttt{data.frame} value. This format is compatible with function
\texttt{geocode()} from package \texttt{ggmap}. It is also possible, to simply pass latitude
...
Spectral objects may have several other attributes used to store metadata, such as the time unit used. There functions available for querying and setting the state if these attributes. \texttt{is\_} functions return a logical value, and \texttt{get} functions return the values of the attributes themselves. In addition \texttt{set} functions can be used to set the value attributes, but many of the \texttt{set} functions are very rarely needed in user code.
...
Sometimes it may be desired to change the time unit used for expressing spectral irradiance or spectral response, and this can be achieved with the \emph{conversion} function \texttt{convertTimeUnit}. This function both converts spectral data to the new unit of expression and sets the \texttt{time.unit} attribute, preserving the validity of the data object.
...
\begin{table}
\caption[Classes for collections of spectral objects]{Classes for collections of spectral objects. Objects of class \texttt{generic\_mspct} can have member objects of any class derived from \texttt{generic\_spct} and can be heterogeneous. Attributes can be queried and set with the normal R methods of the same names. See table \ref{tab:spct:classes} for the attributes used in individual member spectra of collections.}\label{tab:mspct:classes}
\centering
\begin{footnotesize}
\begin{tabular}{>{\ttfamily}l>{\ttfamily}l>{\ttfamily}l}
\toprule
...
\midrule
generic\_mspct & generic\_spct & names, dim, comment \\
...
To convert a `tidy' data frame into a long form spectral object we need to pass the number of spectra through parameter \texttt{multiple.wl} to override the usual check for unique wavelength values.
<<>>=
...
\begin{framed}
\noindent
Methods \texttt{`[`}, and \texttt{`[<-`}, extract and replace `parts' of the collection, respectively. Even when only one member is extracted, the returned value is a collection of spectra. The expected replacement value is also, always a collection of spectra.
\end{framed}
...
\begin{framed}
\noindent
Methods \texttt{`[[`}, \verb|$| and \texttt{`[[<-`}, extract and replace individual members of the collection, respectively. They always return or expect objects of one of the spectral classes.
\end{framed}
...
For our `apply' functions we follow the naming convention used in package \texttt{plyr}, but using  \texttt{ms} as prefix for \texttt{\_mspct} objects. The `apply' functions implemented in the \PB package are \texttt{msmsply}, \texttt{msdply}, \texttt{mslply}  and \texttt{msaply} which both accepts a collections of spectra as first argument and return a collection of spectra, a data frame, a list, or an array respectively (Table \ref{tab:apply}).

\begin{table}
  \caption[Apply functions for collections of spectra]{Apply functions for collections of spectra. Key: v., value returned by `apply' function; f.v., value returned by the applied function (argument \texttt{.fun}). In the table \texttt{generic\_mspct} and \texttt{generic\_spct} indicate objects of these classes or any class derived from them. The exact class of the collection of spectra object returned  will be determined by the class(es) of the values returned by the applied function.}\label{tab:apply}
  \centering
  \begin{scriptsize}
...
Functions \texttt{msmsply()}, \texttt{msdply} and \texttt{mslply} can be used to apply a function to each member spectrum in a collection. The `apply' function to use depends on the return value of the applied function.
...
Above, we demonstrate that \texttt{PAR} is a waveband object, the function \texttt{photobiologyWavebands::PAR()} is a waveband constructor returning a waveband object. See package \PBWB for details on pre-defined waveband constructors for frequently used wavelength ranges and biological spectral weighting functions (BSWFs).
...
\section{Object `inspection' methods}

\subsection{\texttt{print()}}

The \texttt{print()} method for spectra is based on the method defined in package \texttt{dplyr}, consequently, it is possible to use the options from this package to control printing.  In the code chunk below, \texttt{dplyr.print\_max}, the number of rows in the spectral object above which only \texttt{dplyr.print\_min} rows are printed, are both set to 5, instead of the default 20 and 10, to avoid excessive clutter in our examples.
...
For explicit calls to \texttt{print()} its argument \texttt{n} can be used to control the number of lines printed. If \texttt{n} is set to \verb|Inf| the whole spectrum is always printed. The output differs from that of the \texttt{print()} method from package 'dplyr' in that additional metadata specific to spectra are shown.

<<eval=FALSE>>=
print(sun.spct, n = 3)
@

Specialized \texttt{print()} methods for collections of spectra and for \texttt{waveband} objects are also defined.

\subsection{\texttt{summary()}}

The \texttt{summary()} method for spectra is based on base R's \texttt{summary()} method for data frames, and accepts the same arguments. The main difference is that the attributes containing metadata and dimensions of the original spectrum object are copied to the summary object.

<<eval=FALSE>>=
summary(sun.spct)
@

Specialized \texttt{print()} methods for summaries of spectra are defined. The output differs from that of the \texttt{print()} method from base R in that additional metadata specific to spectra are shown.
...
\begin{table}
  ...
  \centering
  \begin{tiny}
  \begin{tabular}{>{\ttfamily}lccccccc}
    \toprule
    \textnormal{methods} & raw/cps & source & response & filter & reflector & object & chroma \\
    \midrule
    merge & + & + & + & + & + & + & + \\
    rbindspct & + & + & + & + & + & + & + \\
    \midrule
...
\begin{table}
  ...
  \begin{tabular}{>{\ttfamily}lccccccc}
    ...
    c & + & + & + & + & + & + & + \\
    \midrule
    \emph{math operators} & ms & ms & ms & ms & ms & ms & ms \\
    \emph{math functions} & ms & ms & ms & ms & ms & ms & ms \\
    \midrule
    e2q, q2e & -- & + & + & -- & -- & -- & -- \\
    A2T, T2A & -- & -- & -- & + & -- & -- & -- \\
    clip\_wl & + & + & + & + & + & + & + \\
...
When \texttt{use.hinges == TRUE} and expansion or replacement is done, two observations are inserted at each boundary, differing in wavelength by $1 \times 10^{-12}$\,nm to prevent rounding errors in later calculations.
...
\begin{table}
  ...
  \begin{tabular}{>{\ttfamily}lccccccc}
    ...
    spread & + & + & + & + & + & + & + \\
    midpoint & + & + & + & + & + & + & + \\
    labels & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} \\
    summary & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} & \textbf{l} \\
    peaks & -- & + & + & + & + & (+) & (+) \\
    valleys & -- & + & + & + & + & (+) & (+) \\
    integrate\_spct & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} \\
    average\_spct & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} & \textbf{a, d, l} \\
    color & -- & + & -- & -- & -- & -- & -- \\
    \bottomrule
  \end{tabular}
  \end{tiny}
...
Specialized definitions of \texttt{summary} and the corresponding \texttt{print} methods are available for spectral objects. Attributes \texttt{"what.measured"}, \texttt{"when.measured"} and \texttt{"where.measured"} are included in the summary print out only if set in the spectral object summarized.
...
\subsubsection{Collections of spectra}\label{sec:collections}
...
and, to integrate a range of wavelengths, in the example, photosynthetically active radiation, we use the waveband \texttt{PAR} we earlier defined.

<<>>=
irrad(sun.spct, PAR)
@

It is also valid to pass as argument for \texttt{w.band} a numeric range representing wavelengths in nanometres.
...
We reuse collection of filter spectra \texttt{filters.mspct} from section \ref{sec:collections} on page \pageref{sec:collections}.
<<>>=
names(filters.mspct)
@
...
and, to obtain the photon fluence for a range of wavelengths, in the example, photosynthetically active radiation, we use the \texttt{PAR} waveband object earlier defined, for 25 minutes of exposure.
...
These indexes are frequently used to summarize reflectance data, for example in remote sensing the NDVI (normalized difference vegetation index). Here we give an \emph{unusual} example to demonstrate that function \texttt{normalized\_diff\_ind()} can be used to calculate, or define any similar index.
...
Here we construct a collection of filter spectra, and then we calculate the transmittance of these filters for two wavebands, obtaining the results as a data frame, with one row per filter, and one column per waveband. We reuse once more \texttt{filters.mspct} from section \ref{sec:collections} on page \pageref{sec:collections}.
...
% \section{Conversion of ``raw'' spectral data}
%
% In this section we cover the final steps of the conversion of raw data. Once the spectral data have been linearised, zero-corrected, and raw counts converted to counts-per-second we can apply the functions included in this package. Earlier steps of the data flow are more dependent on instrument hardware and firmware features and are handled by separate packages---at the moment only package available is \textsl{ooacquire} instruments from Ocean Optics, and which has not yet been released.
%
% At the moment three conversion functions are available: \texttt{cps2irrad()} for irradiance, \texttt{cps2Tfr} for transmittance, and \texttt{cps2Rfr()} for reflectance. These functions expect an instrument descriptor to be present in the \texttt{cps\_spct} objects supplied as arguments.
%
% <<>>=
...
Parameter \texttt{unit.out} can be used to obtain the returned value expressed as time-of-day in hours, minutes, or seconds since midnight.
...
When developing package \PB quite a lot of effort was spent in optimizing performance, especially of the functions accepting vectors as arguments, as in one of our experiments, we need to process several hundred thousands of measured spectra. The defaults should provide good performance in most cases, however, some further improvements are achievable, when a series of different calculations are done on the same spectrum, or when a series of spectra measured at exactly the same wavelengths are used for calculating weighted irradiances or exposures.
...
You can use the package \texttt{microbenchmark} to time the code and find the `regions' that slow it down. I have used it, and also
...
clear.spct & filter\_spct & fraction & ideal transparent filter \\
opaque.spct & filter\_spct & fraction & ideal opaque filter \\
polyester.spct & filter\_spct & fraction & plastic film \\
yellow\_gel.spct & filter\_spct & fraction & theatrical ``gel'' filter \\
clear\_body.spct & object\_spct & fraction & ideal transparent body \\
black\_body.spct & object\_spct & fraction & ideal black body \\
white\_body.spct & object\_spct & fraction & ideal white body \\
photodiode.spct & response\_spct &  A / W  & typical Si photodiode \\
ccd.spct & response\_spct &  A / W & typical CCD array \\
filter\_cps.mspct & cps\_spct & counts / s & example ``raw`` data \\
\bottomrule
\end{tabular}
\end{footnotesize}
\end{table}

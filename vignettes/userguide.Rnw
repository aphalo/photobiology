...
The package defines several classes intended to be used to store different types of spectral data. They are all derived from \texttt{generic\_spct}, which in turn is derived from \texttt{data.frame}. Table \ref{tab:spct:classes} lists them.
...
Before giving examples of how to construct objects to store spectral data we show how to query the class of an object, and how to test the type of an spectrum.
...
We create a data.frame object \texttt{a.spct}, and query its class.
...
Sometimes it may be useful to change the time unit used for expresing spectral irradiance or spectral response, and this can be achieved with function \texttt{convertTimeUnit}.
...
from base R, and its reimplementation from package \texttt{data.frame}, and function \texttt{rbindlist} also
defined in package \texttt{data.frame} should NOT be used. Package \PB provides function \texttt{rbinspct} for
...
\texttt{subset.data.frame} preserve the attributes used by this package. The only difference, in the current implementation,
...
<<>>=
subset.data.frame(sun.spct, s.e.irrad > 0.2)
@
...
\begin{framed}
\noindent
Because of how \verb|[ ]| operators work in R, and especially on objects of classes derived from \texttt{data.frame} some object attributes are lost when this operator is used to subset spct objects, consequently it is safer to use the function described in this section.
\end{framed}
...
\midrule
\textbf{Package \texttt{data.frame}} & & \\
\addlinespace
...
\subsubsection{Irradiances from multiple spectra}
...
\subsection{Calculating fluence}
...
\subsubsection{Normalized difference indexes}

These indexes are frequently used to summarize reflectance data, for example in remote rensing the NDVI (normalized difference vegetation index). Here we give an \emph{unusual} example to demonstrate that function \texttt{normalized\_diff\_ind()} can be used to calculate, or define any similar inedex.
...
\subsubsection{Transmittances from multiple spectra}
...
\subsubsection{Responses from multiple spectra}
...
\begin{framed}
\noindent
It is safe to use function \texttt{subset} with spectral objects as method especialization have been defined, which preserve all attributes including class.

The use of indexing is mostly to be avoided in the current version because attributes defined in the spectral classes are stripped, even though the derived class attribute is preserved. I have been unable to make obvious ways of wrapping this operator work as expected for the spectral classes.

To stack spectral objects the function \texttt{rbindspct} should be used instead of \texttt{rbind} or \texttt{rbindlist} from package \texttt{data.frame}. The functions from package \texttt{data.frame} strip the spectral class attributes from the output, returning always a \texttt{data.frame} object.

\end{framed}

\texttt{subset} methods for spectral objects preserve spectral attributes.

<<>>=
subset(sun.spct, w.length > 600)
subset(sun.spct, c(TRUE, rep(FALSE, 99)))
subset(sun.spct, c(1:5, 10))
selector <- 1:5
subset(sun.spct, selector)
@
...
It is quite easy to calculate `parallel' summary quantities using \texttt{data.frame} syntax and function \texttt{rbindspct}. However, one should be careful with the handling of \texttt{NA} values, and specially make sure that all spectra have values for spectral irradiance at the same wavelengths. Similar code to that given in this section, using \texttt{source\_spct} objects, can be used for \texttt{filter\_spct}, \texttt{reflector\_spct}, and \texttt{response\_spct} objects.
...
\subsection{Calculating times of sunrise, solar noon and sunset}

...
An assumption of the package is that wavelengths are always expressed in nanometres ($1~\Unit{nm} = 1 \cdot 10^{-9} \Unit{m}$). If the data to be analysed uses different units for wavelengths, e.g.\ Angstrom ($1~\Unit{Å} = 1 \cdot 10^{-10} m$), the values need to be rescaled before any calculations. \textbf{Not doing so will yield completely wrong results!}
...
If, for example, the spectral irradiance output by our model of spectroradiometer is in \mwattnm,
...
all functions expect. If wavelength values are in the wrong units, the action-spectra weights and quantum conversions will be wrongly calculated, and the values returned by most functions completely wrong, without warning.

\subsection{Spectral objects}

There are basically three different approaches to the creation of spectra. The first approach consist in setting the class attribute of an existing data frame or data table, in simple terms, converting an existing object into a spectral object. This approach avoids creating a copy of the data, and should be fastest. The second approach is to use an 'as' function to create a new spectral object from a data frame or data table (the original object remains unchanged, and independent of the spectral object). The third approach is to use a function with the same as the spectrum object type, and supply the data as numeric array arguments. With the first two approaches the variables should be suitably named so that they can be recognized, in the third approach the parameter to which the argument vector is supplied determines how it is interpreted.
...
<<>>=
l.spct <- filter.spct(w.length = 300:305, Tpc = rep(100,6))
class.spct(l.spct)
l.spct
@

Of course one can also use existing R objects as arguments.

<<>>=
wl1 <- 300:305
Reflected_fraction <- rep(0.5, 6)
m.spct <- reflector.spct(w.length = wl1, Rfr = Reflected_fraction)
class.spct(m.spct)
m.spct
@
...
The basic math operators have definitions for spectra. It is possible to sum, subtract, multiply and divide spectra. These operators can be used even if the spectral data is on different arbitrary sets of wavelengths. Operators by default use values expressed in energy units.
...
Operators are also defined for operations between an spectrum and a numeric vector (with normal recycling). The current implementation has a limitation: always the first factor or term must be a \texttt{\_\_\_.spct} object for the special operators to be used.
...
Even if the first operand is numeric, the results are calculated as expected, and a spectral object is returned.

<<>>=
2 * sun.spct
@

There is one special case, for \texttt{chroma.spct}: if the second operand is a numeric vector of length three, containing three \emph{named} values `x', `y' and `z', the corresponding element is used for each of the chromaticity `columns' in the \texttt{chroma.spct}. Un-named values or differently named values are not treated specially.
...
The code using \texttt{spct} objects is simple, where \texttt{PAR()} is a predefined waveband constructor.
...
Functions \texttt{e\_irrad} and \texttt{q\_irrad} just save some typing.
...
The recommended practice is to use \texttt{with}, as above. But to keep code lines shorter and simpler we attach the spectrum object \texttt{sun.spct} to make the contained vectors visible. The use of \texttt{w.length} and \texttt{s.e.irrad} and/or \texttt{s.q.irrad} is just our convention, and vectors of any name can be supplied as arguments. And although we here attached a spectrum object, vectors could have been part of a data frame, or just independent numeric vector objects.
...
We attach again the spectrum object.
...
As mentioned above the `ratio' functions for vectors do not accept lists of waveband objects as the `irradiance' functions do. It is possible to use the `irradiance' functions to calculate several ratios in one go, or use the functions that calculate ratios from spectral objects.
...
\subsection{Calculating average transmittance and reflectance}

The functions \texttt{transmittance} and \texttt{reflectance} take filter.spct and reflector.spct objects as arguments, and return an average value for these quantities \textbf{assuming} a light source with a flat spectral energy output. Values expressed as percentages.

<<>>=
transmittance(polyester.new.spct, list(UVB(), UVA(), PAR())) * 100
@

It is more likely the we would like to calculate these values with reference to light of a certain spectral quality. This needs to be calculated by hand, which is not difficult. For example, for UV-B, using energy.

<<>>=
q_irrad(sun.spct * polyester.new.spct, UVB()) / q_irrad(sun.spct, UVB()) * 100
@

And for a list of wavebands, as percentages.

<<>>=
q_irrad(sun.spct * polyester.new.spct, list(UVB(), UVA(), PAR())) /
  q_irrad(sun.spct, list(UVB(), UVA(), PAR())) * 100
@

\subsection{Calculating integrated response}

The functions \texttt{e\_response} and \texttt{q\_response} take response.spct objects as arguments, and return the integrated value for each waveband (integrated over wavelength) \textbf{assuming} a light source with a flat spectral energy or photon output respectively.

If no waveband is supplied as argument, the whole spectrum is intergrated.
...
It is more likely the we would like to calculate these values with reference to light of a certain spectral irradiance. This can be achieved by multiplying the sensor's spectral responsivity by the light source spectral irradiance.
...
<<>>=
q_response(sun.spct * KIPP_PQS1_PAR_quantum.spct, list(UVB(), UVA(), PAR())) * 1e-6
@
...
weighted.s.e.irrad <- s.e.irrad * calc_multipliers(w.length, CIE())
...
A function \verb|interpolate_spectrum| is also included to facilitate interpolation of spectral values. It is used internaly, but can also be used by itself when interpolation is needed.

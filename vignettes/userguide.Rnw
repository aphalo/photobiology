...
By default an ID factor \texttt{spct.idx} is added so that it is recognize the origin of the observations after the binding. If the supplied list has named members, then these names are used as factor levels. Otherwise level names are generated automatically.
...
If a named list with no missing names, is supplied as argument, these names are used for the levels of the ID factor.
...
In the special case when the members of the list are \texttt{source\_spct} objects containing effective spectral irradiance data, and they are not based on the same BSWF, an additional factor \texttt{BSWF} will be added, and the \texttt{BSWF} attribute of the resulting spectrum set to \texttt{"multiple"}.
...
The warning above is caused by the different length of the members of the list, and in this case can be ignored.
...
\subsubsection{Summaries from multiple spectra}

Collections of spectra can be useful not only for time-series of spectra or spectral images, but also when dealing with a small group of related spectra. In the example below we show how to use a collection of spectra for calculating summaries. The spectra in a collection do \textbf{not} need to have been measured at the same wavelength values, or to be of the same length. Consequently, in some cases applying the summary functions described above (except for \texttt{summary}) to collections of spectra can be useful. The value returned is a data frame, with one column per each returned value by the corresponding method for individual spectra.
...
\begin{framed}
Not all summary methods are yet implemented for collections of spectra, as this is a new set of classes, just added to the package.
\end{framed}
...
\subsubsection{Ratios from collections of spectra}

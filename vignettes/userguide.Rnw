...
And then just use the trimmed ($\approx$ sub-setted) data in your calculations.
...
\subsection{Trimming spectra stored as data frames or individual vectors}
...
print(trimmed.na.sun.data)
@

<<example-trim-both-na>>=
trimmed.both.na.sun.data <-
  with(sun.data,
       trim_tails(w.length, s.e.irrad, low.limit = 297, high.limit = 400, fill = NA)
  )
print(trimmed.both.na.sun.data)
@

In addition to \texttt{NA} we can supply an arbitrary numeric value as fill.

<<example-trim-zero>>=
trimmed.zero.sun.data <-
  with(sun.data,
       trim_tails(w.length, s.e.irrad, low.limit = 297, fill = 0.0)
  )
print(trimmed.zero.sun.data)
@

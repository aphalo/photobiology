...
<<>>=
cat(try(h1.spct <- setReflectorSpct(data.table(wl = 5999:6001, Rfr = 1))))
@
...
<<>>=
cat(try(h2.spct <- setReflectorSpct(data.table(wl = 300:305, Rfr = -1))))
@

The constructor converts percents to fractions before applying the tests.

<<>>=
cat(try(h3.spct <- setReflectorSpct(data.table(wl = 300:305, Rpc = -100))))
@

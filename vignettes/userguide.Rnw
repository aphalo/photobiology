...
\sloppy
The generic functions \texttt{print()}, \texttt{min()}, \texttt{max()}, \texttt{range()}, \texttt{mean()} and \texttt{labels()} call the corresponding special functions defined for wavebdand objects: \texttt{print.waveband()}, \texttt{range.waveband()}, etc.

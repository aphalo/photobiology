...
\texttt{generic.spct} objects can be created from data tables and data frames simply by setting them as such. However, a column called \texttt{w.length} must be present and contain wavelength values expressed in nm.
...
<<>>=
e.spct <- setChromaSpct(data.table(w.length = 300:305, x = rep(1,6), y = rep(1,6), z = rep(1,6)))
class(e.spct)
e.spct
@

...
%\VignetteDepends{knitr, photobiology, photobiologyWavebands, photobiologyFilters, ggplot2, ggtern}
...
\subsection{Setting and querying the class of a spectrum object}

\texttt{generic.spct} objects can be created from data tables and data frames simply by setting them as such. However, a column called \texttt{w.length} must be present and contain wavelength values expressed in nm. Functions with names of the form \texttt{is.\_\_\_\_.spct} are defined for all classes of spectra and can take as arguments any R object. In addition function \texttt{is.any.spct} can use to query if an R objetc inherits from any of the classes of spectra defined in this package. Finally function \texttt{class.spct} works similarly to R's \texttt{class} functions but returns only a vector with only the names of spectra classes.
...
\texttt{source.spct} objects can be created from data tables, data frames, and \texttt{generic.spct} simply by setting them as such. However, a columns called \texttt{w.length} (wavelength values expressed in nm) and \texttt{s.e.irrad} (\wattnm) and\textbackslash or \texttt{s.q.irrad} (\molnm) must be present.
...
If the spectral irradiance is expressed per day, then the parameter \texttt{time.unit} should be set to \texttt{"day"} instead of the default of "second". This information can be used when printing the results.
...
We can set an spectrum object to a defferent type.
...
We can also make a `generic.spct' copy of any spectrum object.
...
Method definitions for \texttt{summary} and corresponding \texttt{print} methods are available for spectral objects. In the case of \texttt{source.spct} objects the \texttt{time.unit} attribute makes it possible to print the summary with using the correct units.
...
<<>>=
transmittance(polyester.new.spct, list(UVB(), UVB(), PAR()))
@
...
By default \code{trim\_spct} trims its argument in place, but as \code{sun.spct} is protected as part of the package, we need to first make a copy.

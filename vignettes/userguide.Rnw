...
The default value returned by \texttt{irrad} is in energy irradiance units, unless the option described above
is set.
...
\sloppy
Functions \texttt{e\_irrad} and \texttt{q\_irrad} save some typing, and always return the same type of spectral irradiance quantity, independently of global option \texttt{photobiology.base.unit}.
...
It is more likely that we would like to calculate these values with reference to light of a certain spectral quality. This needs to be calculated by hand, which is not difficult. For example, for UV-B, using energy.

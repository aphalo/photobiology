...
Setting \texttt{strict.range = FALSE} issues a warning instead of an error, and returns an \texttt{.spct} object possibly containing the \emph{bad data}.
...
source.spct & \N & \N &\Y  & \N & \N & waveband (BSWF) & source.spct \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}
\caption[Options]{Options recognized by functions in the photobiology package and the values they can take.\label{tab:options}}
\centering
\begin{footnotesize}
\begin{tabular}{lll}
\toprule
...
\addlinespace
datatable.print.nrows & $n = 100$ & nrow(\emph{spct}) $> n$ ``short'' printing \\
...
\addlinespace
photobiology.radiation.unit & \texttt{"energy"} & (\wattnm) \\
                            & \texttt{"photon"} & (\molnm) \\
photobiology.filter.qty & \texttt{"transmittance"} & ($/1$) \\
                        & \texttt{"absorptance"} & ($/1$) \\
                        & \texttt{"absorbance"} & (a.u. $\log_10$ base) \\
photobiology.use.hinges & \texttt{NULL} & guess automatically \\
                        & \texttt{TRUE} & do not insert hinges \\
                        & \texttt{FALSE} & do insert hinges \\
photobiology.auto.hinges.limit & 0.7 & wavelength step (nm) \\
...
An alternative is as follows, where it is easier to use other aesthetics, or plot addtional curves as shown in the chunk below.
...
Function \texttt{smooth\_spct} is generic with specializations for \texttt{source.spct}, \texttt{filter.spct}, \texttt{reflector.spct}, and \texttt{response.spct}.

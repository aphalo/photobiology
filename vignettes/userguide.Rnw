...
<<>>=
h5.spct <- setReflectorSpct(data.table(wl = 300:305, Rpc = -100),
                            strict.range = FALSE)
class(h.spct)
h.spct
@
...
<<>>=
rbindspct(list(sun.spct, shade.spct), add.factor = TRUE)
@
...
<<>>=
rbindspct(list(sun = sun.spct, shade = shade.spct), add.factor = TRUE)
@

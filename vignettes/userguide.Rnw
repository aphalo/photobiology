...
Spectral objects are printed in the current version of the package by the function defined in package \texttt{dplyr}, consequently, it is possible to the option from this package to control printing. Below \texttt{dplyr.print\_max}, the number of rows above which only `head' rows are printed, is set to 5, instead of the default 20.
...
The functions \texttt{e2q} and \texttt{q2e} can be used on source spectra to convert spectral energy irradiance into spectral photon irradiance and vice versa. A second optional argument sets the action with \texttt{"add"} and \texttt{"replace"} as possible values.

...
The functions \texttt{transmittance}, and \texttt{absorbance} take \texttt{filter.spct} as argument, while function \texttt{reflectance} takes \texttt{reflector.spct} objects as argument. Functions \texttt{transmittance}, \texttt{reflectance} and \texttt{absorptance} are implemented for \texttt{object.spct}. They return as default an average value for these quantities \textbf{assuming} a light source with a flat spectral energy output. Values expressed as percentages can be obtained by setting argument \texttt{pc.out} to \texttt{TRUE}.

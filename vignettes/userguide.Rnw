...
This package defines a family of objects based on data tables (data frame compatible objects) which impose some restrictions on the naming of the vectors, something that allows the second set of functions to `find' the data when passed one of these objects as argument. In addition, as the data is checked when the object is built, there is no need to test for the validity of the data each time a calculation is carried out. The other advantage of using `spct' objects, is that especialized versions of generic functions like \texttt{print} and operators like \texttt{+} can be defined for spectra. \texttt{spct} objects are derived from data.table objects, which in turn are derived from data.frame objects. In this package we define a \textit{generic} spectrum type of object, derived from data table, from which especialized types of spectra are derived. This `parenthood' hierachy means that spectra objects can be used almost anywhere where a \texttt{data.frame} or \texttt{data.table} is expected.

%\begin{framed}
Be aware that although data.tables are syntactically compatible with data.frames, in some special cases the same code may have different semantics as data tables use references in some cases were data frames would use a copy of the data. In general, no such problems exist, and the different semantics only applies to data table especific syntax. If in doubt, to avoid problems, when you really intend to make a new copy of a spectrum, preserving the original object unchanged by later operations on the new `name', use function \texttt{copy} in addition to the assignment operator.
...
% \begin{framed}
% This user guide is out of date. What is included is correct, but most of the object-oriented functionality is not yet described.
% \end{framed}
...
\texttt{source.spct} objects can be created from data tables, data frames, and \texttt{generic.spct} simply by setting then as such. However, a columns called \texttt{w.length} (wavelength values expressed in nm) and \texttt{s.e.irrad} (\wattnm) and\or \texttt{s.q.irrad} (\molnm) must be present.
...
\texttt{filter.spct} objects can be created from data tables, data frames, and \texttt{generic.spct} simply by setting then as such. However, columns called \texttt{w.length} (wavelength values expressed in nm) and \texttt{Tpc} (T\%), \texttt{Tfr} (T as fraction of 1) and\or \texttt{A} (absorbance ($\log_{10}$ based)) must be present.
...
Converting spectra available at a given set of wavelengths values to a different one, is frequently needed when operating with several spectra of different origin. One can increase the \emph{apparent} resolution by interpolation, and reduce it by local averaging or smoothing and resampling. The same function works on all \code{spct} objects, interpolating every column except \code{w.length} and replacing in this last column the old wavelength values with the new ones supplied as argument. The optional argument \code{fill.value} cointrol what value is assigned to wavelengths in the new data that are outside the range of the new waveelngths.
...
Functions \code{integrate_spct} and \code{average_spct} take into account each individual wavelength step, so they return valid results even for spectra measured at arbitrary and varying wavelength steps.
...
The \code{data.table} definition of \code{summary} returns column-wise results that should be interpreted with care if the wavelength step is not constant.
...
The basic math operators have definitions for spectra. It is possible to sum, substract, multiply and divide spectra. These operators can be used even if the spectral data is on different arbitrary sets of wavelengths.
...
\sloppy
The generic functions \texttt{print()}, \texttt{min()}, \texttt{max()}, \texttt{range()}, \texttt{midpoint()} and \texttt{labels()} call the corresponding special functions defined for wavebdand objects: \texttt{print.waveband()}, \texttt{range.waveband()}, etc.
...
Here we describe the functions that use vectors as arguments. In the next section we describe similar functions for \code{spct} objects.

The functions taking numerical vectors as arguments can be used with stored as vectors, or using \code{with} with data frames, data tables, lists, and spectra objetcs.
...
The recommended practice is to use \code{with}, as above. But to keep code lines shorter and simpler we attach the data frame \code{sun.data} to make the contained vectors visible. The use of \code{w.length} and \code{s.e.irrad} and7or \code{s.q.irrad} is just our convention, and vectors of any name can be supplied as arguments.

<<example-data, eval=TRUE>>=
attach(sun.data)
@

Some examples comparing different definitions of wavebands for PAR. Photon irradiance is expresed in \mol.
...
The code using \code{spct} objects is much simpler.
...
\subsection{Weighted spectral irradiance from an unweighted spectrum}
...
In photobiology research we sometimed need to calculate the position on the sun at arbitrary locations and positions. The function \texttt{sun\_angles} returns the azimuth in degrees eastwards, altitude in degrees above the horizon, solar disk diameter in degrees and sun to earth distance in astronomical units. The time should be POSIXct vector, possibly of length one, and it is easiest to use package \texttt{lubridate} for working with time and dates.
...
Examples for spectral as vectors, in this case for the solar spectrum:
...
\begin{framed}
Examples for \code{source.spct} objects to be implemented soon.
\end{framed}
...
Given a color in any of the above ways, yields RGB values that can be used to locate the position of any colour on Maxwell's triangle. Here using R's predifined colours.
...
maxwell.tern <- ggtern(data=test.data, aes(x=R, y=G, z=B, label=colour, fill=colour)) +
...
maxwell.tern <- ggtern(data=test.data, aes(x=R, y=G, z=B, fill=colour, label=labels)) +
...
\subsection{Trimming spectra}

We can use as above \code{low.limit}, \code{high.limit}, and \code{fill} as in the examples above.

<<>>=
trim_spct(sun.spct, low.limit=297)
@

We can in addition do the trimming with a \code{waveband} object.

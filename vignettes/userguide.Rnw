...
maxwell.tern <- ggtern(data=test.data, aes(x=R, y=G, z=B, label=colour)) + geom_point() + geom_text() +
  labs(x = "R", y="G", z="B")
maxwell.tern
@
...
<<>>=
yellow.filter <- with(canary.yellow.new.data, s_e_irrad2rgb(w.length, transmittance, sens=ciexyzCMF2.data))
green.filter <- with(moss.green.new.data, s_e_irrad2rgb(w.length, transmittance, sens=ciexyzCMF2.data))
polyester.filter <- with(polyester.new.data, s_e_irrad2rgb(w.length, transmittance, sens=ciexyzCMF2.data))
colours <- c(yellow.filter, green.filter, polyester.filter)
rgb.values <- col2rgb(colours)
test.data <- data.frame(colour=colours, R=rgb.values[1, ], G=rgb.values[2, ], B=rgb.values[3, ], labels=c("A", "B", "C"))
maxwell.tern <- ggtern(data=test.data, aes(x=R, y=G, z=B, colour=colour, label=labels)) + geom_point(size=6) + geom_text(vjust=-1) +
  labs(x = "R", y="G", z="B")  + scale_colour_identity()
maxwell.tern
@

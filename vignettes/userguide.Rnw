...
In the chuck above, we can see how this works, using in this case the default \code{byref = TRUE} which adds the tags in place, or \texttt{}by reference'', to the spct object supplied as argument.

...
The default for \texttt{irrad}, when no argument \texttt{unit.out} is supplied, is to return the irradiance value in energy irradiance units, unless the R \texttt{photobiology.base.unit} option is set.
...
It is more likely that we would like to calculate these values with reference to light of a certain spectral quality. This needs to be calculated by hand, which is not difficult. For example, for UV-B.

<<>>=
irrad(sun.spct * polyester.new.spct, UVB()) / irrad(sun.spct, UVB()) * 100
@

And for a list of wavebands, as percentages.

<<>>=
irrad(sun.spct * polyester.new.spct, list(UVB(), UVA(), PAR())) /
  irrad(sun.spct, list(UVB(), UVA(), PAR())) * 100
@

...
| `solute_spct`       | `w.length`, `K.mole`       | `K.type`                                            |
|                     | `w.length`, `K.mass`       | `K.type`                                            |
...
data to be analysed use different units for wavelengths, e.g. in Ångstrom
($1~\textrm{Å} = 1 \cdot 10^{-10}\,\mathrm{m}$), the values need to be
re-scaled before creating objects of the spectral classes. The same
...
| x, y, z                     | [0,1]                                   |                        |
| K.mole                      |                                         | K.type = "attenuation" |
| K.mole                      |                                         | K.type = "absorption"  |
| K.mole                      |                                         | K.type = "scattering"  |
| K.mass                      |                                         | K.type = "attenuation" |
| K.mass                      |                                         | K.type = "absorption"  |
| K.mass                      |                                         | K.type = "scattering"  |
| **B: converted**            |                                         |                        |
| wl $\to$ w.length           | nm                                      |                        |
...
objects of class `lubridate:duration` are also accepted.
...
However, methods for querying and setting these attributes are also
available.
...
of mistakes in the use unit scaling factors.\_
...
Finally argument `comment` can be used to add a comment to the data at

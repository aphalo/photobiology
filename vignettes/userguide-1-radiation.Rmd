...
The User Guide is divided into two volumes: *1. Radiation* and *2.
Astronomy*. The second volume also describes functions related to water
...
a convenient and concise syntax tailored for spectral data as in the
...
In addition to the classes for storing individual spectra, classes for
...
on collections of spectra are defined in the package. These methods are
described in a later section and listed in Table 9.

### Summary spectra

Starting from a collection of spectra we may want to obtain a single
spectrum containing a summary of them. The current implementation of the
methods described in this section expects that all spectra in a
collection are of the same class, and data is available for each of them
at exactly the same set of wavelengths.
...
When a normalized spectrum is converted, the normalization is reapplied to
the converted data.
...
When a normalized spectrum is converted, the normalization is not reapplied to
the converted data. _This will change in the future._

### Normalizing a spectrum

Normalization is a way rescaling a spectrum so that the spectral
...
functions with names spelled with "s" instead of "z" are available.)
...
Of course, there are exceptions and for this reason methods used for
...
maximum or minimum and its neighbours.
...
*refined* by fitting a model to describe the peak.
...
algorithm that detects only very narrow peaks, usually called by the
name of spikes. The value returned is as in the case of `peaks` a subset
of the original spectrum. The parameters controlling the sensitivity
also differ.

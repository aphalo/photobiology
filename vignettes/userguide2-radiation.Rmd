...
Functions `when.measured2tb()`, `geocode2tb()`, `lon2tb()`, `lat2tb()` and `what.measured2tb()` extract these same attributes from collection members into a tibble or data frame.  If an existing data frame or tibble with a matching number of rows is passed as second argument the values for the attribute are saved to a new column.
...
By passing the same data frame as argument to successive calls in a sequence, multiple attributes can be retrieved and saved to multiple columns. Here we first compute summary values and then add to the data frame returned by `q_irrad`, attributes retrieved from the original spectral data objects.
...
or data frame. As retrieval of multiple attributes is a frequent operation, we have defined function `add_attr2tb` to simplify code as that shown in the previous code chunk. We here also demonstrate that because the data frame object is passed to the first positional argument, we can use a _pipe_.

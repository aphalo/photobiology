...
It is also valid to pass as argument for `w.band` a numeric range representing wavelengths in nanometres, which is converted into a waveband on-the-fly.

```{r, irrad-3}
irrad(sun.spct, c(400, 700))
```

The default for `irrad`, when no argument `unit.out` is supplied, is to return energy irradiance, unless the R option `photobiology.radiation.unit` is set to `"photon"`.

Functions `e_irrad` and `q_irrad` can save some typing, and always return the same type of spectral irradiance quantity, independently of global option `photobiology.radiation.unit`.
...
```{r, irrad-8a}
irrad(sun.spct, UV_bands.lst, quantity = "total") # watt m-2
```

```{r, irrad-8b}
irrad(sun.spct, UV_bands.lst, quantity = "average") # watt m-2 nm-1
```
...
The naming of the values can be an abbreviation of the physical quantity plus the name of the wavelength, the default as in the examples above, or only the name of the waveband as shown 
below.

```{r}
q_irrad(sun.spct, UV_bands.lst, naming = "short")
```
Naming can also be suppressed. 
...
q_irrad(filtered_sun, list(UVA.wb, PAR.wb))
```

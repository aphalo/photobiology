...
If spectra are stored in _long form_, e.g. for plotting with 'ggplot2', attributes are stored as named lists or enforced to be the uniform across spectra. This allows to reconstruct a collection of spectra from a long-form spectral object preserving most of the metadata.
...
Spectral objects containing multiple spectra identified by a factor. The class of the spectra in the argument must be consistent, as well as certain attributes line `time.unit`, for binding to succeed. Other metadata attributes are retained as named lists.
...
Metadata is set for the individual spectra if it is available in the object being subset. What metadata is automatically stored depends on the version of this package used when creating the long form spectrum object by means of `rbindspct()`. Versions 0.9.14 and later preserve most of the metadata.

```{r, bind-1a}
subset2mspct(two_suns.spct)
```

Data frame containing _tidy_ spectral data (target class and index variable need to be supplied as arguments). A call as the one below adds only default metadata to spectral objects.
...
spectra into a data frame. In the example below we use a collection with only
...
For the most common cases for which one would use the _apply_ functions described in the previous section methods and functions operation on collections of spectra are defined in the package. For example the following statement is equivalent to the second example in the previous section. These are described in a later section and listed in Table 9.
...
Functions `when.measured2tb()`, `geocode2tb()`, `lon_lat2tb()` and `what.measured2tb()` extract these same attributes from collection members into a tibble or data frame.  If an existing data frame or tibble is passed as argumented the attribute values are added as a new column.

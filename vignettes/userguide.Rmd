...
The normalization status of a spectral object can be tested with method `is_normalized()` and the normalization used can be recelled with method `getNormalized()`.
...
Once a spectrum is normalized, summary methods that return values in absolute units such as `irrad()`, will trigger an error if applied. Ratios and similar summaries that are invariant with respect to normalization can be calcualted.

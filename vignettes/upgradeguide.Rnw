...
\section{Changes in versions 0.7.0 and higher}

\texttt{generic\_spct} objects now have an atribute \texttt{"spct.version"} which
help automate reading of \emph{old} objects in future updates. It will also allow
the detection of lost attributes.

The new \texttt{generic\_mspct} objects have an atribute \texttt{"mspct.version"}
which allows the detection of lost attributes.

\section{Changes in versions 0.8.0 and higher}

Although \texttt{data.table} is no longer used internally, objects from versions
6.0 or later do not need to be upgraded. They are fully compatible with the new
version. Of course, data table syntax is not longer accepted, although this should
make no difference as its use converted the spectral objects into data table
objects.

However, use of \texttt{copy()} is no longer needed, as the default behaviour is no longer
to use references. The assignment \texttt{<-} operator, can be used instead. A dummy
\texttt{copy()} function can be defined if needed as:

<<>>=
copy <- function(x) {x}
@

\section{Checking if an object is an \emph{old spectrum}}
...
< 0.6.0. Function \texttt{getObjectVersion} can be used to query the version.
Current version is 1, for older (or corrupted) objects the returned value is
0.
...
\section{Upgrading \emph{old spectra}}
...
Function \texttt{upgrade\_spct} updates the class of old objects, adds the
\texttt{spct.version} attribute and calls \texttt{check()} on the resulting
object. The \emph{upgrade} is done by
...
We can use method \texttt{upgrade\_spct()} to upgrade a single spectrum.
...
We hope that these functions will make the transition to the new versions
less painful.

\section{Updating old scripts}
